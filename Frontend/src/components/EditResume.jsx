...
// import { Check, Download, Loader2, Palette, Trash2 } from 'lucide-react'
...
// import { AlertCircle, ArrowLeft, Save } from 'react-feather'
// import {ProfileInfoForm,EducationDetailsForm ,ContactInfoForm ,CertificationInfoForm ,AdditionalInfoForm  ,ProjectDetailForm ,SkillsInfoForm ,WorkExperienceForm ,} from './Forms'
// import RenderResume from './RenderResume'
// import Modal from './Modal'
// import ThemeSelector from './ThemeSelector'
// import RanderResume from './RenderResume';
...
//   let formData; // define outside so we can log in catch
//   let apiUrl = API_PATHS.RESUME.UPLOAD_IMAGES(resumeId);

//   try {
//     setIsLoading(true);
//     console.log("Starting uploadResumeImages...");
//     console.log("API URL:", apiUrl);

//     // Step 1: Get thumbnail element
...
//     // Step 2: Fix Tailwind colors
//     const fixedThumbnail = fixTailwindColors(thumbnailElement);
//     console.log("Fixed thumbnail created:", fixedThumbnail);
...
//         logging: false,
//         useCORS: true, // optional, may fix image issues
//       });
//       console.log("Canvas generated successfully");
//     } catch (canvasError) {
//       console.error("Error generating canvas:", canvasError);
//       throw canvasError; // rethrow to outer catch
//     }

//     // Step 4: Remove the temporary fixed thumbnail from DOM
//     if (fixedThumbnail.parentNode) {
//       document.body.removeChild(fixedThumbnail);
//     }

//     // Step 5: Convert canvas to file
...
//     let uploadResponse;
//     try {
//       uploadResponse = await axiosInstance.post(apiUrl, formData, {
//         headers: { "Content-Type": "multipart/form-data" },
//       });
//       console.log("Upload response:", uploadResponse);
//     } catch (uploadError) {
//       console.error("Error during API upload:", uploadError);
//       if (uploadError.response) {
//         console.error("Response status:", uploadError.response.status);
//         console.error("Response data:", uploadError.response.data);
//       }
//       throw uploadError; // rethrow so final catch handles it
//     }

//     // Step 8: Update resume details
//     const { thumbnailLink } = uploadResponse.data;
//     console.log("Thumbnail link from server:", thumbnailLink);
...
//     console.log("FormData (if available):", formData);
...
const uploadResumeImages = async () => {
  let formData;
  const apiUrl = API_PATHS.RESUME.UPLOAD_IMAGES(resumeId);
  ...
  try {
    ...
    const originalPosition = thumbnailElement.style.position;
    const originalLeft = thumbnailElement.style.left;
    const originalTop = thumbnailElement.style.top;

    // Temporarily make the element visible but offscreen and hidden (not visible to user)
    ...
    // Now get bounding rect
    const originalRect = thumbnailElement.getBoundingClientRect();
    console.log("Original thumbnail rect:", originalRect);
    ...
    if (originalRect.width === 0 || originalRect.height === 0) {
      ...
      thumbnailElement.style.position = originalPosition;
      thumbnailElement.style.left = originalLeft;
      thumbnailElement.style.top = originalTop;
      throw new Error("Thumbnail element has zero width or height");
    }

    // Clone and fix Tailwind colors (your custom function)
    const fixedThumbnail = fixTailwindColors(thumbnailElement.cloneNode(true));

    // Prepare the cloned element to be offscreen visible for html2canvas
    fixedThumbnail.style.position = "absolute";
    fixedThumbnail.style.left = "-9999px";
    fixedThumbnail.style.top = "0";
    fixedThumbnail.style.display = "block";
    fixedThumbnail.style.visibility = "visible";
    fixedThumbnail.style.width = originalRect.width + "px";
    fixedThumbnail.style.height = originalRect.height + "px";

    // Ensure all children are visible
    fixedThumbnail.querySelectorAll("*").forEach(el => {
    ...
    console.log("Fixed thumbnail prepared for capture");
    ...
    // Restore original styles on the original thumbnail element
    thumbnailElement.style.display = originalDisplay;
    thumbnailElement.style.visibility = originalVisibility;
    thumbnailElement.style.position = originalPosition;
    thumbnailElement.style.left = originalLeft;
    ...
    if (!thumbnailDataUrl || thumbnailDataUrl.length < 50) {
      throw new Error("Generated thumbnail is empty — cannot upload");
    }
...
// so all this function will be used for edit resume 
  ...
  <div className={containerStyles.main}>
    <div className={containerStyles.header}>
      ...
      <div className='flex flex-wrap items-center gap-3'>
        ...
        <button className={buttonStyles.delete} disabled={isLoading} onClick={handleDeleteResume}>
    ...
    <div className={containerStyles.grid}>
      <div className={containerStyles.formContainer}>
        ...
        <div className=' p-4 sm:p-6'>
          ...
          <div className=' flex  flex-wra items-center justify-end gap-3'>
            ...
            <button className={buttonStyles.next} onClick={validateAndNext} disabled={isLoading}>{currentPage === "additionalInfo" && <Download size={16}></Download>}
            {currentPage === 'additionalInfo' ? "Preview & Download":"Next"}
            {currentPage === "additionalInfo" && <ArrowLeft size={16} className=' rotate-180'/>}</button>
          </div>
        </div>
      </div>
  ...
  <Modal isOpen={openThemeSelector} onClose={()=>setOpenThemeSelector(false)} title="Change Title">
    <div className={containerStyles.modalContent}>
      <ThemeSelector selectedTheme={resumeData?.template.theme} setSelectedTheme={updateTheme} onClose={()=>setOpenThemeSelector(false)}></ThemeSelector>
    </div>
  </Modal>
  ...
  actionBtnIcon={
    isDownloading ? (
      <Loader2 size={16} className=' animate-spine'/>
    ):(
      downloadSuccess ? (
        <Check size={16} className=' text-white'></Check>
      ):(
