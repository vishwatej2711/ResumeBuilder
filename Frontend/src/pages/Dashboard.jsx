...
//it will show if completed or filled ot will do ++
...
const handleDeleteResume = async () => {
  if (!resumeToDelete) return; // ✅ Correct: only exit if nothing to delete
  ...
  try {
    ...
    setShowDeleteConfirm(false); // ✅ This will now run!
  }
};
        ...
        <div className={styles.container}>
            <div className={styles.headerWrapper}>
                <div >
                    ...
                    <p className={styles.headerSubtitle}>{allResume.length>0 ? `You have ${allResume.length} resumes${allResume.length !== 1? 's' : ''}`:'Start building your professional resume'}
                ...
                <div className='flex gap-4'>
                    <button className={styles.createButton} onClick={()=>setOpenCreateModal(true)}>
                        ...
                        <span className={styles.createButtonContent}>Create Now
                            <LucideFilePlus className='group-hover:translate-x-1 transition-transform' size={18}></LucideFilePlus>
                        </span>
                    </button>
                </div>
            </div>
            ...
            {!loading && allResume.length ===0 &&(
                <div className={styles.emptyIconWrapper}>
                    <div className={styles.emptyIconWrapper}>
                    <LucideFilePlus size={32} className=' text-violet-600'></LucideFilePlus>
                    </div>
                    ...
                    <button className={styles.createButton} onClick={()=>{setOpenCreateModal(true)}}>
                        ...
                        <span className={styles.createButtonContent}>Create Your First Resume
                            <LucideFilePlus className='group-hover:translate-x-1 transition-transform' size={20}></LucideFilePlus>
                        </span>
                    </button>
                </div>
            )}
        ...
        <div className='p-4'>
            <div className='flex flex-col items-center text-center'>
                <div className={styles.deleteIconWrapper}>
                    <LucideTrash2 className='text-orange-600' size={24}></LucideTrash2>
                </div>
